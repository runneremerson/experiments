...
main(int argc, const char *argv[])
{
    ...
    struct Stat parents_stat;
    memset(&parents_stat, 0, sizeof(parents_stat));
    int ret = zoo_exists(zkhandle, "/election", 0, &parents_stat); 
    if(ret==ZNONODE){
        int create_ret = zoo_create(zkhandle, "/election",
                         "parents",
                         7,
                         &ZOO_OPEN_ACL_UNSAFE,
                         0,
                         ...
                         sizeof(buf)-1);
        if (create_ret) {
            fprintf(stderr, "zoo_create error [%d], %s\n", create_ret, zerror(create_ret));
            exit(EXIT_FAILURE);
        }
    }else{
        if (ret) {
